...
def test_operator():
    with KopfRunner(['run', '--namespace shutdown-operator', '--verbose', 'app/kopf_operator.py']) as runner:
        ...
        subprocess.run("kubectl apply -f test/deployment.yaml -n shutdown-operator", shell=True, check=True)
        time.sleep(30)  # give it some time to react and to sleep and to retry
        
        subprocess.run("kubectl apply -f test/statefulset.yaml -n shutdown-operator", shell=True, check=True)
        time.sleep(30)  # give it some time to react and to sleep and to retry
        
        subprocess.run("kubectl apply -f test/daemonset.yaml -n shutdown-operator", shell=True, check=True)
        ...
        subprocess.run("kubectl delete -f test/daemonset.yaml -n shutdown-operator", shell=True, check=True)
        time.sleep(30)  # give it some time to react
        
        subprocess.run("kubectl delete -f test/statefulset.yaml -n shutdown-operator", shell=True, check=True)
        
        time.sleep(30)  # give it some time to react
        
        subprocess.run("kubectl delete -f test/deployment.yaml -n shutdown-operator", shell=True, check=True)
